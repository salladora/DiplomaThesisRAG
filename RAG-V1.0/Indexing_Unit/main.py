...
def process_files():
    """Main pipeline: Lists files, chunks, embeds, and uploads."""
    ...
    for doc_id, file_key in enumerate(file_keys, start=1):
        ...
        chunks = chunking_from_s3(s3_bucket_name, file_key)
        if not chunks:
            continue
        embeddings = embedding(chunks)
        vectors_with_metadata = list_vectors_with_metadata(embeddings, chunks, doc_id)
